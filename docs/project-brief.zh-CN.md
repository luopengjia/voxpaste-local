...
VoxPaste Local 是一个本地优先的 macOS 语音输入工具：按住快捷键或鼠标侧键录音，松开后使用本地 Whisper 转写，可选本地 LLM 润色，并自动粘贴到当前光标。
...
- 处理 macOS 权限、快捷键监听、鼠标侧键按住触发、Whisper 模型加载、可选 LLM 润色等细节。
...
- 独立实现 macOS 本地语音输入工作流：按住快捷键或鼠标侧键录音，松开后通过 MLX Whisper 转写，并自动粘贴到当前光标。
